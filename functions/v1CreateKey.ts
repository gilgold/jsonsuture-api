...
Deno.serve(async (req) => {
  ...
  let bodySource = '';
  try { const body = await req.clone().json(); bodySource = typeof body?.source === 'string' ? body.source : ''; } catch { /* body is optional */ }
  const querySource = new URL(req.url).searchParams.get('source') || '';
  const candidate = (querySource || bodySource).toLowerCase().replace(/[^a-z_]/g, '').slice(0, 32);
  const acquisitionSource = SOURCES.has(candidate) ? candidate : candidate ? 'other' : 'direct';

  const key = randomKey();
  const keyHash = await sha256(key);
  const prefix = key.slice(0, 15);
  const billingPeriod = new Date().toISOString().slice(0, 7);
  await base44.asServiceRole.entities.ApiKey.create({ key_hash: keyHash, key_prefix: prefix, plan: 'free', status: 'active', monthly_limit: 250, rate_limit_per_minute: 10, billing_period: billingPeriod, monthly_used: 0, created_ip_hash: ipHash, acquisition_source: acquisitionSource });
  console.log(JSON.stringify({ level: 'info', event: 'signup', request_id: requestId, key_prefix: prefix, acquisition_source: acquisitionSource }));
