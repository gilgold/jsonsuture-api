...
const PRICE_IDS = {
  developer: 'price_1U6d9MPHVKJSy1xYzXJJ3fQ8',
  pro: 'price_1U6d9MPHVKJSy1xY8CH2Pcvp',
} as const;
...
Deno.serve(async (req) => {
  ...
  if (!secret) return json({ ok: false, error: { code: 'BILLING_NOT_CONFIGURED', status: 503, detail: 'Paid upgrades are temporarily unavailable; the free tier remains active.' } }, 503);

  const stripe = new Stripe(secret);
  const session = await stripe.checkout.sessions.create({ mode: 'subscription', line_items: [{ price: PRICE_IDS[plan], quantity: 1 }], client_reference_id: key.id, subscription_data: { metadata: { key_hash: keyHash, plan } }, metadata: { key_hash: keyHash, plan }, success_url: 'https://vesper-3159a405.base44.app/functions/jsonSutureHome?checkout=success', cancel_url: 'https://vesper-3159a405.base44.app/functions/jsonSutureHome?checkout=cancelled', allow_promotion_codes: true });
